...
extern "C" {
    ...
    void Java_com_example_hellojni_DualBitmap_copyBuf(JNIEnv* env, jlong in, jlong out) {
        CBitmapHandle *hin = (CBitmapHandle*)in;
        CBitmapHandle *hout = (CBitmapHandle*)out;
        memcpy(hout->pixels, hin->pixels, sizeof(uint32_t)*(hin->width)*(hin->height));
    }
